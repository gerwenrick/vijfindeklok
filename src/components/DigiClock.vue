<template>
  <div class="clock-single" :id="timezone.tzCode">
    <div class="ana-clock">
      <svg
        id="clock"
        class="svg-clock"
     ...
     c0.028,0.439-0.015,1.152-0.097,1.584l-3.712,19.623c-0.082,0.433-0.508,0.786-0.948,0.786H242.515z"
        ...
        <path
         ...
         L247.862,249.75z"
        />
        <g :id="`seconds${setTz}`" class="seconds">
          <line
            fill="none"
            stroke="#BF4116"
            stroke-miterlimit="10"
    ...
    <ul>
      <li id="label">Label: {{ timezone.label }}</li>
      <li id="offset">Offset: {{ timezone.offset }}</li>
      <li id="tzCode">tzCode: {{ timezone.tzCode }}</li>
      <li class="timeTitle">{{ timezone.tzCode }}</li>
      <li>
        <span>{{ setTime }}</span>
      </li>
    </ul>
  ...
  methods: {
    thisTime() {
      window.setInterval(() => {
        ...
        let thisTz = this.timezone.tzCode;
        ...
        let hourArrow = document.getElementById("hours" + thisTz);
        let minArrow = document.getElementById("minutes" + thisTz);
        let secArrow = document.getElementById("seconds" + thisTz);

        let localTimeSec = DateTime.local().setZone(thisTz).second;
        let localTimeMin = DateTime.local().setZone(thisTz).minute;
        let localTimeHour = DateTime.local().setZone(thisTz).hour;

        let hourDeg = localTimeHour * 30 + localTimeMin * 0.5;
        let minDeg = localTimeMin * 6 + localTimeSec * 0.1;
        let secDeg = localTimeSec * 6;

        hourArrow.style.transform = `rotateZ(${hourDeg}deg)`;
        minArrow.style.transform = `rotateZ(${minDeg}deg)`;
        secArrow.style.transform = `rotateZ(${secDeg}deg)`;

        this.setTime = localTime;

        let tzDiv = document.getElementById(thisTz);

        if (localTime.indexOf(checkTime) > -1) {
          tzDiv.style.backgroundColor = "rgba(46, 204, 113, 1)";
        } else {
          tzDiv.style.backgroundColor = "rgba(231, 76, 60, 0)";
        }
      }, 1000);
    }
  },
...
@import "../styles/components/_digiclock.scss";
</style>
